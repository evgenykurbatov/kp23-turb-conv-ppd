...
print("dotM_eff = %.2e [M_sol/yr]" % (dotM_eff/(const.M_sol/const.yr)))
...
plt.suptitle(("'%s': " % fname) \
             + (r"$t = %g$ [yr], " % (t/const.yr)) \
             + (r"$R = %g$ [AU], $\alpha_\mathrm{SS} = %.2e$" % (R/const.AU, alpha_SS)))
...
ax_.set_ylabel(r"$\mathsf{Ra}$")
ax_.set_ylim(ymin=1e9)
...
ax_ = ax[1,0]
ax_.loglog(z/const.AU, f['rho'])
ax_.set_ylabel(r"$\rho$ [g cm$^{-3}$]")
"""
ax_ = ax[1,0]
...
ax_ = ax[3,0]
ax_.semilogx(f['z']/const.AU, f['rho']*(f['S_uv'] + f['S_ext']), label=r"$\rho (S_\mathrm{uv} + S_\mathrm{ext})$")
ax_.semilogx(f['z']/const.AU, f['rho']*f['S_conv'], label=r"$\rho S_\mathrm{conv}$")
ax_.semilogx(f['z']/const.AU, f['rho']*f['S_turb'], label=r"$\rho S_\mathrm{turb}$")
...
ax_.semilogx(z/const.AU, f['rho']*f['g'] - np.gradient(f['wzz'], z), 'k', label=r"$\rho g - dw_{zz}/dz$")
