...
class Model:
    ...
    def heating_callback(self, T):
        ...
        ##
        ## Average the heating function to avoid oscillations

        ## Average over time
        self.S_hist = np.roll(self.S_hist, -1)
        self.S_hist[-1] = S
        S_avg_t = np.empty_like(S)
        for i in range(S.size):
            S_avg_t[i] = self.S_hist[:,i].mean()
        ## Average  over space
        h = 4
        #S_wide = np.empty(S.size + 2*h)
        S_wide = np.zeros(S.size + 2*h)
        #S_wide[:h] = S[:h:-1]
        S_wide[h:(h+S.size)] = S_avg_t
        #S_wide[(h+S.size):] = 0.0
        S_mean = np.empty_like(S)
        for i in range(S.size):
            for j in range(-h, h+1):
                S_mean[i] = S_wide[h+i+j]/(2.0*h+1.0)
        self.S_conv = S_mean.copy()

        #dS_dT = dSconv_dT
        dS_dT = np.zeros_like(S)

        S_mean += self.S_uv + self.S_ext
...
def init(model):
    ...
    model.S_hist  = np.zeros((5, model.sigma.size))
...
if __name__ == '__main__':
    ...
    ## Path to data folder
    model.fpath = 'conv_avg_dotM=1e-7'
